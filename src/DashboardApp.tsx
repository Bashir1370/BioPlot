    ...
    <aside className="home-sidebar">
      <div className="home-sidebar-top">
        ...
        <nav className="home-nav" aria-label={t.home}>
          ...
          <a href="#projects" aria-label={t.projects}><Icon name="projects"/><span>{t.projects}</span>{projectList.length > 0 && <em>{projectList.length}</em>}</a>
    ...
    <main className="home-main">
      ...
      <div className="home-content">
        ...
        <section className="home-section" id="projects" aria-labelledby="recent-projects-title">
          <div className="home-section-heading projects-heading"><div><span className="home-section-kicker">YOUR WORK</span><h2 id="recent-projects-title">{showAllProjects ? t.allProjects : t.recent}</h2><p>{t.recentSub}</p></div>{lastProject && <button className="home-continue" onClick={() => openProject(lastProject)}><Icon name="clock"/>{t.continueWork}</button>}{projectList.length > 6 && <button className="home-text-button" onClick={() => setShowAllProjects(value => !value)}>{showAllProjects ? t.recent : t.viewAll}<Icon name="arrow" size={15}/></button>}</div>

          {loadState === 'loading' && <div className="home-project-grid">{[0,1,2].map(item => <ProjectSkeleton key={item}/>)}</div>}

          {loadState === 'ready' && projectList.length === 0 && <div className="home-empty-state"><div className="empty-illustration"><StudioIcon name="canvas" size={48} weight="duotone"/></div><div><span className="home-section-kicker">NEW WORKSPACE</span><h3>{t.noProjects}</h3><p>{t.noProjectsText}</p><div><button className="home-primary-cta compact" onClick={() => createFromTemplate('blank')}><Icon name="plus"/>{t.createFigure}</button><a href="#templates" className="home-secondary-cta compact"><Icon name="templates"/>{t.browseTemplates}</a></div></div></div>}
