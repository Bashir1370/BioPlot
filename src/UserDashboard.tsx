...
import {List,X,SidebarSimple,House,SquaresFour,Clock,User,Gear,SignOut,Plus,ArrowRight,CloudArrowUp} from '@phosphor-icons/react';
...
export function UserDashboard(){
 ...
 const create=()=>run(async()=>{const doc=createBlankDocument();doc.metadata.locale=locale;await projects.save(doc);window.location.href=`/editor?id=${encodeURIComponent(doc.id)}`;});
 ...
 const filtered=items.filter(row=>row.title.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase()));
 ...
 return <div className={`workspace ${collapsed?'is-collapsed':''}`} dir={fa?'rtl':'ltr'}>
  ...
  <main className="workspace-main">
   ...
   {(tab==='figures'||tab==='activity')&&<>
    <section className="workspace-stats"><article><SquaresFour/><strong>{items.length}</strong><span>{text('Saved figures','شکل ذخیره‌شده')}</span></article><article><Clock/><strong>{recent}</strong><span>{text('Updated this week','ویرایش این هفته')}</span></article><article><User/><strong>{account.user?text('Account','حساب کاربری'):text('Device','این دستگاه')}</strong><span>{text('Storage workspace','فضای ذخیره‌سازی')}</span></article></section>
    {loading?<p role="status">{text('Loading figures…','در حال دریافت شکل‌ها…')}</p>:error?<div role="alert" className="workspace-notice">{error}<button onClick={()=>void refresh()}>{text('Retry','تلاش دوباره')}</button></div>:tab==='activity'?<section className="workspace-activity">{items.length===0?<p>{text('No saved activity yet.','هنوز فعالیت ذخیره‌شده‌ای ندارید.')}</p>:items.map(row=><a key={row.id} href={`/editor?id=${encodeURIComponent(row.id)}`}><Clock/><div><strong>{row.title}</strong><small>{text('Last saved','آخرین ذخیره')} · {date(row.updatedAt)}</small></div><ArrowRight/></a>)}</section>:<>
     <div className="workspace-tools"><input type="search" aria-label={text('Search figures','جستجوی شکل‌ها')} placeholder={text('Search your figures…','جستجوی شکل‌های شما…')} value={query} onChange={event=>setQuery(event.target.value)}/>{account.user&&<button disabled={busy} onClick={()=>void importDrafts()}><CloudArrowUp size={18}/>{text('Move device drafts to account','انتقال طرح‌های دستگاه به حساب')}</button>}</div>
     {filtered.length===0?<div className="workspace-empty"><SquaresFour size={44}/><h2>{query?text('No matching figures','شکلی پیدا نشد'):text('Your next figure starts here','شکل بعدی شما از اینجا شروع می‌شود')}</h2>{!query&&<button className="workspace-primary" disabled={busy} onClick={()=>void create()}>{text('Create a figure','ساخت شکل')}</button>}</div>:<section className="workspace-figures">{filtered.map(row=><article key={row.id}><a href={`/editor?id=${encodeURIComponent(row.id)}`} className="workspace-preview">{thumbs[row.id]?<img src={thumbs[row.id]} alt={row.title} loading="lazy"/>:<SquaresFour size={42}/>}</a><div className="workspace-card-copy"><a href={`/editor?id=${encodeURIComponent(row.id)}`}><h2>{row.title}</h2></a><small>{date(row.updatedAt)}</small><div><button disabled={busy} onClick={()=>rename(row)}>{text('Rename','تغییر نام')}</button><button disabled={busy} onClick={()=>void duplicate(row)}>{text('Duplicate','ساخت کپی')}</button><a href={`/editor?id=${encodeURIComponent(row.id)}`}>{text('Continue editing','ادامه ویرایش')} <ArrowRight size={15}/></a></div></div></article>)}</section>}
    </>}
   </>}
