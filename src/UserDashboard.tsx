...
import {List,X,SidebarSimple,House,SquaresFour,Clock,User,Gear,SignOut,Plus,ArrowRight,CloudArrowUp,MagnifyingGlass,Flask,ArrowUpRight} from '@phosphor-icons/react';
 ...
 return <div className={`workspace ${collapsed?'is-collapsed':''}`} dir={fa?'rtl':'ltr'}>
  ...
  <main className="workspace-main">
   ...
   {(tab==='figures'||tab==='activity')&&<>
    {tab==='figures'&&<section className="workspace-welcome"><div><span className="workspace-eyebrow"><Flask size={17}/>{text('YOUR SCIENCE, BEAUTIFULLY TOLD','روایت تصویری علم شما')}</span><h2>{text('Make room for your next discovery.','به کشف بعدی‌تان تصویر بدهید.')}</h2><p>{text('Pick up a figure or start with a fresh canvas. Your work is saved automatically after your first edit.','طرح‌هایتان را ادامه دهید یا با یک بوم تازه شروع کنید. ذخیره خودکار پس از اولین ویرایش آغاز می‌شود.')}</p><a href="/templates">{text('Explore templates','مشاهده قالب‌ها')}<ArrowUpRight size={18}/></a></div><div className="workspace-orbit" aria-hidden="true"><span/><span/><span/><Flask weight="duotone"/></div></section>}
