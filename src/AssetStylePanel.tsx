...
export function AssetStylePanel({fa,object,onChange,onBrowse}:{fa:boolean;object:StyledAssetObject;onChange:(label:string,next:StyledAssetObject)=>void;onBrowse:()=>void}){
  ...
  const autoOpenChecked=useRef<string|null>(null);
  useLayoutEffect(()=>{
    if(autoOpenChecked.current===object.id)return;
    autoOpenChecked.current=object.id;
  ...
  return <div className="asset-style-panel">
    ...
    <section className="asset-style-section">
      <div className="asset-style-section-head"><div><b>{fa?'رنگ‌بندی':'Style'}</b><small>{fa?'انتخاب رنگ با حفظ سایه‌روشن':'Quick presets'}</small></div><button onClick={()=>onChange('Reset asset style',resetAssetVisualStyle(object,original?.svg,originalColors))}>{fa?'بازنشانی':'Reset styles'}</button></div>
      ...
      </div>:<p className="asset-style-empty">{fa?'برای این شکل، رنگ‌بندی آماده تعریف نشده است.':'No color presets are configured for this asset.'}</p>}
    </section>
