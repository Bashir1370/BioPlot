...
export const HOME_TEXT_DEFAULTS: HomeContent = {
 en: {eyebrow:'BIOPLOT FIGURE STUDIO',title:'Your science.',highlight:'Clearly illustrated.',description:'Turn complex biology into clear, editable, publication-ready figures. One focused workspace for your next discovery.',primaryText:'Create scientific figure',secondaryText:'Browse templates',socialProof:'Native vector export · Structured autosave · Persian + English'},
 fa: {eyebrow:'BIOPLOT FIGURE STUDIO',title:'علم شما؛',highlight:'روشن و تماشایی.',description:'ایده‌های پیچیده زیستی را به شکل‌هایی روشن، قابل‌ویرایش و آماده انتشار تبدیل کن؛ در یک فضای طراحی علمی.',primaryText:'ساخت شکل علمی',secondaryText:'مشاهده قالب‌ها',socialProof:'خروجی وکتور واقعی · ذخیره ساختاریافته · فارسی + انگلیسی'},
};
export function normalizeHomeContent(value: unknown): HomeContent {
 const input = value as Partial<HomeContent> | null;
 return Object.fromEntries((['en','fa'] as const).map(locale => [locale,Object.fromEntries(HOME_TEXT_FIELDS.map(key => [key,typeof input?.[locale]?.[key] === 'string' ? input[locale]![key] : HOME_TEXT_DEFAULTS[locale][key]]))])) as HomeContent;
}
