...
export function EditorStudio(){
  ...
  const fitCanvas=()=>{
    ...
    if(viewport) setZoom(Math.max(.1,Math.min(1,(viewport.clientWidth-64)/page.width,(viewport.clientHeight-64)/page.height)));
  };
  useEffect(()=>{
    const viewport=canvasViewportRef.current;
    if(!viewport)return;
  ...
  const columns=`0px ${libraryCollapsed?0:libraryWidth}px 5px minmax(0,1fr) 5px ${inspectorCollapsed?0:inspectorWidth}px`;

  return <div className={`studio-shell ${libraryCollapsed?'library-closed':'library-open'} ${inspectorCollapsed?'inspector-closed':'inspector-open'}`} dir={fa?'rtl':'ltr'}>
    <header className="studio-topbar"><a className="studio-brand" href="index.html"><span>B</span><strong>BioPlot</strong><em>Figure Studio</em></a><div className="studio-document"><a href="index.html" className="studio-icon-btn" aria-label={fa?'بازگشت به پروژه‌ها':'Back to projects'}><StudioIcon name="arrow"/></a><div><input value={documentState.title} onChange={event=>setTitle(event.target.value)}/><small><i/>{saveState}</small></div></div><div className="studio-history"><button disabled={!storeRef.current.history.canUndo} onClick={()=>storeRef.current.undo()} title={fa?'واگرد':'Undo'} aria-label={fa?'واگرد':'Undo'}><StudioIcon name="undo"/></button><button disabled={!storeRef.current.history.canRedo} onClick={()=>storeRef.current.redo()} title={fa?'ازنو':'Redo'} aria-label={fa?'ازنو':'Redo'}><StudioIcon name="redo"/></button></div><div className="studio-top-actions"><button onClick={toggleLocale}>{fa?'EN':'FA'}</button><button onClick={share}><StudioIcon name="share"/>{shareState==='Share'&&fa?'اشتراک':shareState}</button><button onClick={()=>{setInspectorCollapsed(false);setInspectorTab('quality');}}><StudioIcon name="check"/>{fa?'بررسی':'Check'}</button><button className="studio-export" onClick={()=>{setInspectorCollapsed(false);setInspectorTab('export');}}><StudioIcon name="download"/>{fa?'خروجی':'Export'}</button></div></header>

    <div className="studio-command-deck">
      <div className="studio-command-row">
        <nav className="studio-library-tabs" aria-label={fa?'ابزارهای طراحی':'Design tools'}>
          {(['assets','elements','upload'] as Panel[]).map(key=><button key={key} className={panel===key&&!libraryCollapsed?'active':''} aria-pressed={panel===key&&!libraryCollapsed} onClick={()=>{if(panel===key&&!libraryCollapsed)setLibraryCollapsed(true);else{setPanel(key);setLibraryCollapsed(false);}}}><StudioIcon name={key}/><span>{key==='assets'?(fa?'کتابخانه':'Library'):key==='elements'?(fa?'اجزا':'Elements'):(fa?'آپلود':'Upload')}</span></button>)}
        </nav>
