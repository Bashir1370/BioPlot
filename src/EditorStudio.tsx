...
function Elements({fa,addObject}:{fa:boolean;addObject:(object:BioPlotObject)=>void}){
  return <div className="studio-elements"><section><h3>{fa?'متن و برچسب':'Text & labels'}</h3><div><button onClick={()=>addObject(makeText())}><span>T</span><b>{fa?'متن علمی':'Scientific text'}</b></button><button onClick={()=>addObject(makePanelLabel())}><span>A</span><b>{fa?'برچسب پنل':'Panel label'}</b></button><button onClick={()=>addObject(makeTagLabel())}><span>Tag</span><b>{fa?'برچسب':'Tag label'}</b></button><button onClick={()=>addObject(makeNoteLabel())}><span>Note</span><b>{fa?'یادداشت علمی':'Note'}</b></button></div></section><section><h3>{fa?'شکل‌ها':'Shapes'}</h3><div><button onClick={()=>addObject(makeShape('rect'))}><span>□</span><b>{fa?'مستطیل':'Rectangle'}</b></button><button onClick={()=>addObject(makeShape('ellipse'))}><span>○</span><b>{fa?'بیضی':'Ellipse'}</b></button><button onClick={()=>addObject(makeContainer())}><span>▣</span><b>{fa?'کانتینر':'Container'}</b></button></div></section><section><h3>{fa?'فلش و اتصال':'Arrows & connectors'}</h3><div><button onClick={()=>addObject(makeArrow())}><span>→</span><b>{fa?'فلش':'Arrow'}</b></button><button onClick={()=>addObject(makeConnector('straight'))}><span>↦</span><b>{fa?'اتصال مستقیم':'Straight'}</b></button><button onClick={()=>addObject(makeConnector('elbow'))}><span>⌟</span><b>{fa?'اتصال زاویه‌دار':'Elbow'}</b></button><button onClick={()=>addObject(makeConnector('curved'))}><span>⌒</span><b>{fa?'اتصال منحنی':'Curved'}</b></button></div></section></div>;
}
