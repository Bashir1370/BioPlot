...
export function EditorStudio(){
  ...
  useEffect(()=>{
    const onKey=(event:KeyboardEvent)=>{
      ...
      if(event.key==='Escape'){setSelected(new Set());setAssetEditOpen(false);setContextMenu(null);return;}
  ...
  function beginMarquee(event:ReactPointerEvent<HTMLDivElement>){if(event.target!==event.currentTarget||!artboardRef.current)return;const art=artboardRef.current.getBoundingClientRect(),sx=(event.clientX-art.left)/zoom,sy=(event.clientY-art.top)/zoom,original=event.shiftKey?new Set(selected):new Set<string>();if(!event.shiftKey){setSelected(new Set());setAssetEditOpen(false);}const move=(pointer:PointerEvent)=>{const rect=rectFromPoints(sx,sy,(pointer.clientX-art.left)/zoom,(pointer.clientY-art.top)/zoom);setMarquee(rect);setSelected(new Set([...original,...objectsInRect(objects,rect).filter(object=>!object.locked).map(object=>object.id)]));};const up=()=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up);setMarquee(null);};window.addEventListener('pointermove',move);window.addEventListener('pointerup',up,{once:true});}
