import {startOrderFromFigure} from './orders/orderSource';
import { categoryLabel, projectTitleInput } from './editorLabels';
...
export function EditorStudio({onReady}:{onReady?:()=>void}={}){
  ...
  function beginMarquee(event:ReactPointerEvent<HTMLDivElement>){
    ...
    event.preventDefault();event.stopPropagation();
    artboardRef.current.focus({preventScroll:true});
    const art=artboardRef.current.getBoundingClientRect();
  ...
  return <div className={`studio-shell ${libraryCollapsed?'library-closed':'library-open'} ${inspectorCollapsed?'inspector-closed':'inspector-open'} ${pagesCollapsed?'pages-closed':'pages-open'} ${selectedObjects.length?'has-selection':'no-selection'} ${showGrid?'grid-visible':''}`} dir={fa?'rtl':'ltr'}>
    ...
    <header className="studio-topbar">
      <div className="reference-document-card">
        ...
        <div className="studio-document"><input aria-label={fa?'نام پروژه':'Project name'} placeholder={fa?'نام پروژه را وارد کنید…':'Enter a project name…'} value={projectTitleInput(documentState.title)} onChange={event=>setTitle(event.target.value)}/><small title={saveState}><i/>{saveState}</small>{saveFailed&&<button onClick={()=>setSaveAttempt(value=>value+1)}>{fa?'تلاش دوباره':'Retry save'}</button>}</div>
      ...
      <div className="studio-top-actions"><button title={fa?'ارسال نسخه‌ای از طرح فعلی برای برآورد هزینه؛ پیش از ثبت سفارش چیزی بارگذاری نمی‌شود.':'Prepare a snapshot for a design quote. Nothing is uploaded until submission.'} onClick={()=>{try{startOrderFromFigure(documentState);}catch{window.alert(fa?'آماده‌سازی طرح انجام نشد. فایل را خروجی بگیرید و در بخش سفارش طراحی پیوست کنید.':'Could not prepare a snapshot. Export your figure and attach it to a design request.');}}}>{fa?'سفارش تکمیل طرح':'Request design help'}</button><button onClick={toggleLocale}>{fa?'EN':'FA'}</button><button className="reference-check" title={fa?'بررسی کیفیت':'Publication check'} onClick={()=>{setInspectorCollapsed(false);setInspectorTab('quality');}} aria-label={fa?'بررسی کیفیت':'Publication check'}><StudioIcon name="check"/></button><button onClick={share}><StudioIcon name="share"/>{shareState==='Share'&&fa?'اشتراک':shareState}</button><button className="studio-export" onClick={()=>{setInspectorCollapsed(false);setInspectorTab('export');}}><StudioIcon name="download"/>{fa?'خروجی':'Export'}</button></div>
    </header>
    {bounds&&selectionToolbarVisible&&<FloatingSelectionToolbar viewportRef={canvasViewportRef} artboardRef={artboardRef} label={fa?'ویرایش انتخاب':'Edit selection'}>
      <div className="studio-command-row">
        {selectedAsset&&<button disabled={!!selectedAsset.locked} onClick={()=>setVectorAsset(structuredClone(selectedAsset))}>{fa?'ویرایش':'Edit'}</button>}
