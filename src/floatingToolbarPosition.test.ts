...
describe('context toolbar screen placement',()=>{
 it('centers above selection with a gap for rotation handles',()=>{
  expect(place({left:400,top:300,right:500,bottom:400},viewport,size)).toEqual({left:290,top:192,visible:true});
 });
 it('flips below objects near the top and clamps at either horizontal edge',()=>{
  expect(place({left:105,top:55,right:150,bottom:100},viewport,size)).toEqual({left:110,top:118,visible:true});
