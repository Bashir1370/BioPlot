import {shapeSvgBody} from './shapeGeometry';
...
export function EditorObjectView({ object, objects, selected, onPointerDown, onContextMenu }:{
  ...
  if (rendered.type === 'label') return <div {...common} className={`${common.className} studio-label label-${rendered.variant}`} style={{...style,color:rendered.color,background:rendered.background,borderColor:rendered.borderColor,borderRadius:rendered.frameShape==='circle'?'50%':rendered.frameShape==='rect'?0:8,whiteSpace:'pre-wrap',lineHeight:1.2,padding:8,justifyContent:rendered.align==='left'?'flex-start':rendered.align==='right'?'flex-end':'center',fontSize:rendered.fontSize,fontWeight:rendered.fontWeight,fontFamily:fontStackForText(rendered.text,rendered.fontFamily),textAlign:rendered.align}}>{rendered.text}</div>;
  if (rendered.type === 'shape') return <div {...common}><svg width="100%" height="100%" viewBox={`0 0 ${rendered.width} ${rendered.height}`} dangerouslySetInnerHTML={{__html:shapeSvgBody(rendered)}}/></div>;
  ...
  if (rendered.type === 'asset') {
    ...
    return <div {...common} className={`${common.className} studio-asset ${asset.lineAsset?'bp-line-asset':''}`}><div className="studio-asset-visual" style={{filter:assetCssFilter(asset)}} dangerouslySetInnerHTML={{__html:assetSvgWithTint(asset)}}/></div>;
  }
