...
export interface LabelObject extends BaseObject { type:'label'; text:string; variant:'panel'|'tag'|'note'; frameShape?:'rect'|'rounded'|'circle'; color:string; background:string; borderColor:string; fontSize:number; fontWeight:number; fontFamily?:string; align:'left'|'center'|'right'; }
export interface ShapeObject extends BaseObject { type:'shape'; shape:'rect'|'rounded'|'ellipse'|'circle'|'triangle'|'diamond'|'pentagon'|'hexagon'|'star'|'trapezoid'|'parallelogram'|'cross'; fill:string; stroke:string; strokeWidth:number; lineStyle?:LineStyle; radius:number; }
...
export interface AssetObject extends BaseObject { type:'asset'; lineAsset?:boolean; assetId:string; svg:string; colors:AssetColorSlot[]; tintColor?:string; paletteColor?:string; }
