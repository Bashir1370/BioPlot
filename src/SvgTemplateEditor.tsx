...
type EditorState={undo:boolean;redo:boolean;name:string;nodes:boolean;node:boolean;count:number;layers:Array<{element:Element;name:string;selected:boolean}>};
...
export class SvgEditingSession {
  ...
  refresh=()=>{
    ...
    this.changed({undo:this.index>0,redo:this.index<this.history.length-1,name:this.selected.map(e=>e.getAttribute('aria-label')||e.id||e.localName).join(', '),nodes:this.nodes,node:!!this.node&&!this.node.control,count:this.selected.length,layers:layers.map(element=>({element,name:element.getAttribute('aria-label')||element.id||element.localName,selected:this.selected.includes(element as SVGGraphicsElement)}))});
  };
...
export function SvgTemplateEditor({svg,fa,onApply,onClose}:{svg:string;fa:boolean;onApply:(svg:string)=>void;onClose:()=>void}){
  ...
  const initialize=()=>run(()=>{const doc=frame.current?.contentDocument;if(!doc)return;session.current?.dispose();session.current=new SvgEditingSession(doc,sanitizeSvg(svg),setState);});
  ...
  return <dialog ref={dialog} className="svg-editor-dialog" dir={fa?'rtl':'ltr'} onCancel={event=>{event.preventDefault();close();}} onKeyDown={event=>{event.stopPropagation();if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='z'&&!(event.target instanceof HTMLInputElement)){event.preventDefault();session.current?.restore(event.shiftKey?1:-1);}}}>
    ...
    <div className="svg-editor-workspace"><aside><strong>{t('Components','اجزای قالب')}</strong><div className="svg-editor-layers">{state.layers.map((layer,i)=><button key={i} className={layer.selected?'active':''} onClick={event=>session.current?.pick(layer.element,event.shiftKey)}>{layer.name}</button>)}</div><div className="svg-editor-add">{button('+ Rectangle','+ مستطیل',()=>session.current?.add('rect'))}{button('+ Ellipse','+ بیضی',()=>session.current?.add('ellipse'))}{button('+ Text','+ متن',()=>session.current?.add('text'))}<label>{t('Insert SVG','افزودن SVG')}<input type="file" accept=".svg,image/svg+xml" onChange={async e=>{const file=e.target.files?.[0];if(file){try{const text=await file.text();run(()=>session.current?.addSvg(text));}catch{setError(t('Cannot read file','خواندن فایل ممکن نشد'));}}e.target.value='';}}/></label><label>{t('Text content','متن جزء انتخاب‌شده')}<input aria-label="Component text" key={state.name} defaultValue={session.current?.selected[0]?.localName==='text'?session.current.selected[0].textContent??'':''} disabled={session.current?.selected[0]?.localName!=='text'} onBlur={e=>run(()=>session.current?.setText(e.target.value))}/></label></div></aside>
