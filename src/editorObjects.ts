...
export function makeText(x = 360, y = 270): TextObject {
  return { id: makeId(), type: 'text', name: 'Scientific text', text: 'Scientific annotation', x, y, width: 240, height: 54, rotation: 0, opacity: 1, color: '#17303f', fontSize: 16, fontWeight: 600, fontStyle: 'normal', fontFamily: 'Inter', lineHeight: 1.25, letterSpacing: 0, textDecoration: 'none', align: 'center', verticalAlign: 'middle' };
}
export function makePanelLabel(x = 90, y = 90, text = 'A'): LabelObject {
  return { id: makeId(), type: 'label', name: `Panel ${text}`, text, variant: 'panel', x, y, width: 42, height: 42, rotation: 0, opacity: 1, color: '#17303f', background: '#ffffff', borderColor: '#cfdce1', fontSize: 18, fontWeight: 800, fontFamily: 'Inter', align: 'center' };
}
export function makeTagLabel(x = 360, y = 270): LabelObject {
  return { id: makeId(), type: 'label', name: 'Scientific label', text: 'Label', variant: 'tag', x, y, width: 110, height: 34, rotation: 0, opacity: 1, color: '#0b6d69', background: '#eaf7f5', borderColor: '#b9dfda', fontSize: 13, fontWeight: 700, fontFamily: 'Inter', align: 'center' };
}
export function makeNoteLabel(x = 350, y = 260): LabelObject {
...
export function makeConnector(route: ConnectorObject['route'] = 'straight', x = 390, y = 300): ConnectorObject {
  return { id: makeId(), type: 'connector', name: `${route[0].toUpperCase()}${route.slice(1)} connector`, x, y, width: 170, height: route === 'straight' ? 40 : 90, rotation: 0, opacity: 1, stroke: '#526e7a', strokeWidth: 2.5, lineStyle: 'solid', route, arrowHead: 'end', fromPort: 'auto', toPort: 'auto', autoRoute: true };
}
...
export function makeImage(src: string, name = 'Image', x = 340, y = 230, naturalWidth?:number, naturalHeight?:number): ImageObject {
  return { id: makeId(), type: 'image', name, src, alt: name, fit: 'contain', naturalWidth, naturalHeight, x, y, width: 280, height: 200, rotation: 0, opacity: 1 };
}
