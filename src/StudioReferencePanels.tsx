...
export function AssetCatalogView({fa,assets,grouped,favorites,onAdd,onFavorite,onCategory}:{fa:boolean;assets:ScientificAsset[];grouped:boolean;favorites:Set<string>;onAdd:(asset:ScientificAsset)=>void;onFavorite:(id:string)=>void;onCategory:(category:string)=>void}){
  ...
  const card=(asset:ScientificAsset)=><div className="asset-card-wrap" key={asset.id}><button className="asset-main" onClick={()=>onAdd(asset)} title={asset.name} aria-label={fa?`افزودن ${asset.name}`:`Add ${asset.name}`}><span dangerouslySetInnerHTML={{__html:asset.svg}}/></button><button className={`asset-favorite ${favorites.has(asset.id)?'active':''}`} aria-pressed={favorites.has(asset.id)} aria-label={fa?`علاقه‌مندی: ${asset.name}`:`Favorite ${asset.name}`} onClick={()=>onFavorite(asset.id)}><StudioIcon name="star"/></button></div>;
